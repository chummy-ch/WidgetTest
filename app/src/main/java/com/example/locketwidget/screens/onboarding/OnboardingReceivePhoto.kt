...
@Composable
fun OnboardingReceivePhotoScreen(
    next: () -> Unit
) {
    OnboardingBody(
        imageContent = {
            Box(modifier = Modifier.fillMaxHeight(0.5f)) {
                Image(
                    painter = painterResource(R.drawable.onboarding_image_4),
                    contentDescription = null,
                    contentScale = ContentScale.Crop
                )
            }
        },
        title = getAnnotatedString(stringResource(R.string.onboarding_receive_photo_title)),
        text = stringResource(R.string.onboarding_receive_photo_text),
